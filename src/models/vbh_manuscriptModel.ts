...
export const submitAuthorDetails = async (data: {
  ...
  const [result] = await pool.query(
    'INSERT INTO manuscript_vbh (author_name, author_email, author_designation, author_organization, author_mobile, userId, submission_date) VALUES (?, ?, ?, ?, ?, ?, NOW())',
  ...
  for (const coAuthor of data.co_authors) {
    await pool.query(
      'INSERT INTO co_authors_vbh (manuscriptId, name, email, designation, organization, mobile) VALUES (?, ?, ?, ?, ?, ?)',
...
export const updateManuscriptFile = async (manuscriptId: number, filePath: string, user_id: string) => {
  await pool.query(
    'UPDATE manuscript_vbh SET file_path = ?, userId = ? WHERE id = ?',
...
export const updateArticleDetails = async (manuscriptId: number, details: any, user_id: string) => {
  await pool.query(
    'UPDATE manuscript_vbh SET title = ?, abstract = ?, category = ?, keywords = ?, userId = ? WHERE id = ?',
